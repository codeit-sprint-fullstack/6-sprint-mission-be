...
ImageRouter.post(
  ...
  asyncErrorHandler(async (req: MulterRequest, res: Response): Promise<void> => {
    if (!req.file) {
      res.status(400).json({ error: '이미지가 업로드되지 않았습니다.' });
      return;
    }

    const filePath = path.join('static/images/', req.file.filename);
    res.send({ url: `${process.env.BASE_URL}/${filePath}` });
  }),
);
