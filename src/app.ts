...
import imageRouter from "./routes/image.route"; // 이 라우터는 로컬 파일 처리였다면 S3 처리로 수정하거나 필요시 제거
...
const upload = multer({
  storage: multerS3({
    ...
    contentType: multerS3.AUTO_CONTENT_TYPE, // 파일의 MIME 타입 자동 감지
    metadata: function (_: any, file, cb) {
...
app.use("/images", imageRouter); // 이 라우터가 로컬 파일 처리에 의존하고 있었다면 수정 필요
