...
app.delete(
    ...
    asyncErrorHandler(async (req, res) => {
        ...
        return res.status(200).json({ message: '해당 게시글이 삭제되었습니다.' });
    }),
);
...
app.get(
    '/articles',
    asyncErrorHandler(async (req, res) => {
        let parsedQuery = create(req.query, GetArticleListRequestStruct);

        const { cursor, take, orderBy, word } = parsedQuery;
